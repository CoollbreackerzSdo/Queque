...
//Test de rendimiento
[MemoryDiagnoser, MaxColumn, MinColumn, IterationsColumn,IterationCount(1_000)]
public class SearchTester
{
    //Setup
    [IterationSetup]
    [GlobalSetup]
    public void BinarySearchTesterSetup()
    {
        //Creamos una colección de números de 10,000 elementos
        _collectionDeNúmeros = Enumerable.Range(1, 10_000).ToArray();
    }
    //Tests
    [Benchmark]
    public void BúsquedaLineal()
        => _consumer.Consume(CollectionSearch.Lineal(_collectionDeNúmeros!, _principalValue));
    [Benchmark]
    public void BúsquedaBinaria()
        => _consumer.Consume(CollectionSearch.Binary(_collectionDeNúmeros!, _principalValue));
    [Benchmark]
    public void BúsquedaBinariaRecursiva()
        => _consumer.Consume(CollectionSearch.BinaryRecursive(_collectionDeNúmeros, _principalValue, 0, _collectionDeNúmeros!.Length));
    [Benchmark]
    public void BúsquedaBinariaNativa()
        => _consumer.Consume(Array.BinarySearch(_collectionDeNúmeros!, 0, _collectionDeNúmeros!.Length, _principalValue));
    [Benchmark]
    public void BúsquedaIndexOf()
        => _consumer.Consume(Array.IndexOf(_collectionDeNúmeros!, _principalValue));

    //Propiedades
    /// <summary>
    /// Consumidor de memoria principal
    /// </summary>
    private readonly Consumer _consumer = new();
    /// <summary>
    /// Valor principal a buscar
    /// </summary>
    [Range(0,10_000)]
    private int _principalValue { get; set; }
    /// <summary>
    /// Array principal de datos
    /// </summary>
    private int[]? _collectionDeNúmeros;
}                               
